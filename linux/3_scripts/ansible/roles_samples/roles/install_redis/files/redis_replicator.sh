...
#bind 127.0.0.1
requirepass KTHKuJ3jER9m
protected-mode yes
port $((6378 + $redis_instance_num))
tcp-backlog 511
timeout 0
tcp-keepalive 300
daemonize no
...
rdbchecksum yes
dbfilename dump.rdb
dir /var/lib/redis$redis_instance_numi
masterauth KTHKuJ3jER9m
...
[Unit]
Description=Redis$redis_instance_num persistent key-value database
After=network.target
[Service]
ExecStart=/usr/bin/redis-server /etc/redis$redis_instance_num.conf --supervised systemd
...
for (( redis_instance_num=2; redis_instance_num <= $cpu_count; redis_instance_num++ ))
do
service redis$redis_instance_num start
done
